...
helm repo update
helm install joplin-server djjudas21/joplin-server
```
...
helm install joplin-server \
  --set env.TZ="America/New York" \
    djjudas21/joplin-server
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install joplin-server djjudas21/joplin-server -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/djjudas21/joplin-server?modal=changelog)
